...
def get_clip_runs(log_root):
    ...
    for d in log_root.iterdir():
        ...
        # Matcher : PPO_clip_0.1_1, PPO_clip_0.2_1, PPO_clip_0.3_1
        m = re.match(r"^PPO_clip_([0-9.]+)_(\d+)$", d.name)
        if m:
            clip = float(m.group(1))
            run_id = int(m.group(2))
            runs.append((clip, run_id, d))

    if not runs:
        raise FileNotFoundError(
            f"Aucun dossier PPO_clip_*_<n> trouvé dans {log_root}\n"
...
def plot_tag(runs, tag: str, out_dir: Path, smooth_window: int = 200):
    """Trace et sauvegarde un graphe brut + lissé pour un tag donné."""
    fig, axes = plt.subplots(1, 2, figsize=(16, 5))
    fig.suptitle(f"{tag} — comparaison des clip_range", fontsize=13)

    found_any = False

    for clip, run_id, run_path in runs:
...
def main():
    ...
    log_root = root / "logs"
    out_dir = root / "compare_clip_plots"
    out_dir.mkdir(exist_ok=True)

    runs = get_clip_runs(log_root)
    print(f"{len(runs)} run(s) trouvé(s) :")
    for clip, run_id, path in runs:
        print(f"  clip={clip}  run_id={run_id}  → {path.name}")
