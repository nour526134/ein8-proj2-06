...
def plot_individual(rewards_dir: Path, out_dir: Path):
    for i, json_file in enumerate(sorted(rewards_dir.glob("rewards_gamma_*.json"))):
        gamma = float(json_file.stem.replace("rewards_gamma_", ""))
        ...
        ax.set_title(f"Reward par step — gamma = {gamma}", fontsize=13)
        ax.set_xlim(left=0)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()

        save_path = out_dir / f"step_reward_gamma_{gamma}.png"
...
def plot_comparison(rewards_dir: Path, out_dir: Path):
    ...
    for i, json_file in enumerate(sorted(rewards_dir.glob("rewards_gamma_*.json"))):
        ...
        ax.plot(steps, rewards, alpha=0.7, linewidth=0.8,
                color=COLORS[i % len(COLORS)], label=f"gamma = {gamma}")
        print(f"  gamma={gamma} : {len(steps)} steps chargés")
        found_any = True
    ...
    ax.set_xlabel("Step", fontsize=12)
    ax.set_ylabel("Reward", fontsize=12)
    ax.set_title("Reward par step — comparaison des gamma", fontsize=13)
    ax.set_xlim(left=0)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=11)
    fig.tight_layout()

    save_path = out_dir / "step_reward_per_gamma.png"
    fig.savefig(save_path, dpi=200)
    plt.close()
    print(f"Sauvegardé : {save_path}")
