...
class GTFSService:
    ...
    def __init__(self, gtfs_dir= None):
        if gtfs_dir is None:
            # Chemin absolu basé sur l'emplacement de ce fichier (src/gtfs_service.py)
            # -> remonte d'un niveau (src -> racine du projet) puis descend dans data/
            gtfs_dir = Path(__file__).resolve().parent.parent / "data" / "gtfs_bordeaux"
    ...
    def _load_all(self):
        ...
        self._stop_times = pd.read_csv(self._csv("stop_times"), dtype={"trip_id": str,"stop_id": str,"stop_sequence": int})
        ...
        try:
            ...
            self._routes = None
        # dictionnaire avec les stop_id -> ensemble de trip_ids
        self._stop_to_trips = (
    ...
    def load_stops(self) -> dict:
        ...
        for _, row in self._stops.iterrows():
            sid = str(row["stop_id"])
            if not (43.5 <= float(row["stop_lat"]) <= 45.5 and -1.5 <= float(row["stop_lon"]) <= 0.3):
                continue
            if sid in active_stops:
                # Ignorer les stops Car TER : même position que Train TER mais moins de trips
                if "Car TER" in sid:
                    continue
                result[sid] = {
    ...
    def get_reachable_stations(self, origin_stop_id) -> pd.DataFrame:
        """Retourne un DataFrame des stations atteignables depuis `origin_stop_id` (même trip, stop_sequence > celui de l'origine)."""
    ...
    def train_wait_time_from_trips(
        ...
        Si le dernier départ de la journée est déjà passé,
        ...
        if len(future) > 0:
            ...
            MAX_WAIT = 120.0  # on ne considère pas attendre plus de 2h
